  ...
  return (
    <div className="w-full">
      <div className="py-20 md:py-28 bg-gradient-to-b from-blue-500/5 to-transparent border-b border-white/5 mb-12 md:mb-16 text-center md:text-left px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <span className="inline-block text-xs font-bold tracking-[0.12em] uppercase text-accent-cyan mb-3">Join Us</span>
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-20">
        ...
        <div className="glass-card flex flex-col md:flex-row gap-6 md:gap-8 items-start">
          ...
          <div className="flex-1">
            <h3 className="text-xl md:text-2xl mb-2 font-semibold">Interested in Joining URJA Lab?</h3>
        ...
        {opportunities.length > 0 && (
          <div className="mt-16 md:mt-20">
            <h2 className="text-2xl md:text-3xl mb-8">Current Openings</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {opportunities.map((opp, index) => (
                <div key={index} className="glass-card flex flex-col hover:border-accent-blue/30 transition-colors group">
                  <div className="flex flex-col sm:flex-row sm:align-start justify-between gap-4 mb-4">
                    <h4 className="text-lg font-semibold m-0 group-hover:text-accent-cyan transition-colors">{opp.title || 'Position Title'}</h4>
                    {opp.type && <span className="shrink-0 text-xs font-semibold uppercase tracking-wider px-3 py-1 rounded-full bg-accent-blue/10 text-accent-cyan border border-accent-blue/25 self-start">{opp.type}</span>}
                  </div>
                  {opp.description && <p className="text-sm md:text-base text-slate-400 flex-1 mb-6 leading-relaxed">{opp.description}</p>}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-white/5 mt-auto">
                    ...
                    {opp.link && (
                      <a href={opp.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm font-semibold text-accent-cyan font-space hover:gap-2 hover:text-blue-300 transition-all self-end sm:self-auto">
